...
if(mysqli_num_rows($sql) > 0 ) {
	$msg="<center><h2><font color='#FF0000'>You have already been voted, No need to vote again</h2></center></font>";
		include 'ASSEMBLY ELECTION.php';
		exit();	
}
else{
$sql1 =mysqli_query($con, 'UPDATE languages SET votecount = votecount +1 WHERE fullname = "'.$_SESSION['SESS_NAME'].'"');

$sql =mysqli_query($con, 'UPDATE languages SET about = "'.$_POST['lan'].'" WHERE fullname="'.$_SESSION['SESS_NAME'].'"');
...
$sql3 = mysqli_query($con, 'UPDATE voters SET voted= "'.$_POST['lan'].'" WHERE username="'.$_SESSION['SESS_NAME'].'"');
	...
	$msg="<center><h2><font color='#FF0000'>Congratulation, you have made your vote.</h2></center></font>";
	include 'ASSEMBLY ELECTION.php';
	exit();
	}
}
